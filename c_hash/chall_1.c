#include "helpers.h"
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

uint8_t lfsr113() {
  static unsigned int z1 = 0xdeadbeef, z2 = 0xf001, z3 = 0x700001, z4 = 0x12345;
  unsigned int b;
  b = ((z1 << 6) ^ z1) >> 13;
  z1 = ((z1 & 4294967294U) << 18) ^ b;
  b = ((z2 << 2) ^ z2) >> 27;
  z2 = ((z2 & 4294967288U) << 2) ^ b;
  b = ((z3 << 13) ^ z3) >> 21;
  z3 = ((z3 & 4294967280U) << 7) ^ b;
  b = ((z4 << 3) ^ z4) >> 12;
  z4 = ((z4 & 4294967168U) << 13) ^ b;
  return (z1 ^ z2 ^ z3 ^ z4);
}
void rotate(uint8_t *out, uint8_t *in, size_t in_size, uint32_t n,
            uint32_t direction) {
  n = n % in_size;
  /* printf("rotating by %d\n", n); */
  if (direction)
    for (unsigned int i = 0; i < n; i++) {
      out[in_size - i - 1] = in[i];
      /* printf("{1} out[%lu] = in[%d]\n", in_size - i - 1, i); */
    }
  else
    for (unsigned int i = 0; i < n; i++) {
      out[i] = in[in_size - i - 1];
      /* printf("{1} out[%d] = in[%lu]\n", i, in_size - i - 1); */
    }

  if (direction)
    for (unsigned int i = 0; i < in_size - n; i++) {
      out[in_size - n - i - 1] = in[in_size - i - 1];
      /* printf("{2} out[%lu] = in[%lu]\n", in_size - n - i - 1, in_size - i -
       * 1); */
    }
  else
    for (unsigned int i = 0; i < in_size - n; i++) {
      out[n + i] = in[i];
      /* printf("{2} out[%d] = in[%d]\n", n + i, i); */
    }
}

void xor
    (uint8_t * out, uint8_t *in, size_t n) {
      for (size_t i = 0; i < n; i++)
        out[i] = out[i] ^ in[i];
    }

    void add(uint8_t *out, uint8_t *in, size_t n) {
  for (size_t i = 0; i < n; i++)
    out[i] = out[i] + in[i];
}

void sbox(uint8_t *out, uint8_t *in, size_t n) {
  uint8_t sbox[256] = {
      // AES SBOX for example
      };

  for (size_t i = 0; i < n; i++)
    out[i] = sbox[in[i] ^ lfsr113()];
}

void hash_round(uint8_t *out, uint8_t *in, size_t n) {
  uint8_t i0 = in[0];
  uint8_t i1 = in[n - 1];
  uint8_t *buf = malloc(n + 1);
  rotate(buf, in, n, i1, i0 + i1);
  rotate(out, in, n, i0, i1 * i0);

  xor(out, buf, n);
  add(out, buf, n);
  sbox(out, out, n);

  free(buf);
}

void padding(uint8_t *buf, size_t n, size_t limit) {
  for (size_t i = n; i < limit; i++) {
    /* printf("\nn=%lu ++ buf[%lu] = buf[%lu] * buf[%lu] padding value %d\n", n,
     * i, */
    /* i - n, i - 1, buf[i - n] * buf[i - 1]); */
    buf[i] = buf[i - n] * buf[i - 1] % 0xff;
  }
}
void start_challenge1() {
  size_t hash_size = 16;
  uint8_t *buf = malloc(1024);
  size_t input_size;
  printf("[/] Heres the secret message try to figure it out, by reversing the "
         "algorithem \n");
  input_size = user_input_bytes(
      "[*] You can test the algorithem with your input .... [max size 1024]",
      buf);
  uint8_t *chunk = malloc(hash_size);
  uint8_t *state = malloc(hash_size);
  memset(chunk, 0, hash_size);
  memset(state, 0, hash_size);

  printf("You Entered %lu bytes: ", input_size);
  print_bytes(buf, input_size);
  printf("\n");

  padding(buf, input_size, hash_size);
  if (input_size < hash_size)
    input_size = hash_size;
  for (size_t feed = 0; feed < input_size / hash_size; feed++) {
    memcpy(chunk, buf + feed * hash_size, hash_size);
    xor(state, chunk, hash_size);

    for (size_t i = 0; i < hash_size; i++) {
      hash_round(state, state, hash_size);
    }
  }
   printf("Result :  ");
   print_bytes(state, hash_size); 
  printf("\n");

  free(state);
  free(chunk);
  free(buf);
}
